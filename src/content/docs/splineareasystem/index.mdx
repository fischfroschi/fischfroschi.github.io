...
| Event | Provides | Description |
|---|---|---|
| `StartMoving` | `Moving Actor` | Fires when the plane begins traveling the track; provides a reference to the moving actor |
| `OnValidAreaStateChanged` | `Is in Valid Zone` | Fires when the plane enters or exits the valid drop zone |
| `OnMovingEnded` | - | Fires when the plane finishes the current track pass |

Override `StartMoving` to get a reference to the plane actor - attach particles, play audio, or start a camera pan. Use `OnValidAreaStateChanged` with `Is in Valid Zone` to open or close the drop window in your game logic. Override `OnMovingEnded` to react when the pass is complete.
...
| `RayIntersectionsWithSplineBorder` | `RayOrigin`, `RayDirection`, `bBidirectional` | Returns all intersections between a ray and the spline border |

### BP_SplineAreaZone

| Setting | Type | Description |
|---|---|---|
| `Spline Area` | ASplineAreaActor | Map boundary reference |
| `Num Iterations` | Integer | Number of shrink passes to run |
| `Radius Variance` | Float | Per-iteration circle size randomization |
| `Radius Curve` | CurveFloat | Zone circle radius profile over shrink progress |
| `Time Curve` | CurveFloat | Shrink timing and speed profile |
...
| Variable | Type | Description |
|---|---|---|
| `IterationTransforms` | Transform | Transforms generated for the current iteration |
| `IterationIndex` | Integer | Index of the currently running iteration |
...
|---|---|---|
| `Spline Area` | ASplineAreaActor | Map boundary reference |
| `Velocity` | Float | Plane movement speed along the track |
| `Area Start Offset Length` | Float | Approach offset outside the map polygon |
| `Area Stop Offset Length` | Float | Exit offset before the far polygon edge |
| `Valid Area Start Offset` | Float | Drop zone open offset inside the map polygon |
| `Valid Area Stop Offset` | Float | Drop zone close offset before the far edge |
| `Point Interval` | Float | Spacing between generated points on the track path |
| `Height Curve` | CurveFloat | Flight height profile along the track |

| Event | Provides | Description |
|---|---|---|
| `StartMoving` | `Moving Actor` | Fires when the plane begins traveling the track |
| `OnValidAreaStateChanged` | `Is in Valid Zone` | Fires when the plane enters or exits the valid drop zone |
| `OnMovingEnded` | - | Fires when the plane finishes the current track pass |

### BP_SplineAreaZoneNotificationComponent

| Event | Provides | Description |
|---|---|---|
| `OnValidAreaStateChanged` | `Is in Valid Zone` | Fires when the owning actor enters or exits the valid zone |